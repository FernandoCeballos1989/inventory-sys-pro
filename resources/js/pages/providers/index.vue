...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            <div class="w-1/3">
                <Link :href="providersRoute.create().url">
            ...
            <div class="w-10/12 justify-center">
                <Table>
                    <TableCaption
                        >A list of your recent Providers.</TableCaption
                    >
                    <TableHeader>
                        <TableRow>
                    ...
                    <TableBody>
                        <TableRow
                            v-for="provider in providerProps.providers.data"
                            :key="provider.id"
                        >
                            ...
                            <TableCell class="text-right">
                                <Link
                                    :href="providersRoute.edit(provider.id).url"
                                >
                                    <Button variant="outline">Edit</Button>
                                </Link>
                                <Link
                                    :href="
                                        providersRoute.destroy(provider.id).url
                                    "
                                    method="delete"
                                    class="ml-2"
                                    @click="confirmDelete"
                ...
                <div class="mt-4 flex items-center justify-between">
                    <p class="text-sm text-muted-foreground">
                        Página {{ providerProps.providers.current_page }} de
                        {{ providerProps.providers.last_page }}
                    </p>
                    <div class="flex gap-2">
                        <Link
                            v-if="providerProps.providers.prev_page_url"
                            :href="providerProps.providers.prev_page_url"
                        >
                            <Button variant="outline">Previous</Button>
                        </Link>
                        <Link
                            v-if="providerProps.providers.next_page_url"
